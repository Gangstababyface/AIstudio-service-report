...
export const ReportEditor: React.FC<EditorProps> = ({ reportId, userEmail, userName, onClose }) => {
  ...
  if (loading) return <div className="p-10 flex justify-center bg-slate-950"><div className="animate-spin-slow text-4xl text-red-500"><i className="fa-solid fa-gear"></i></div></div>;

  return (
    <div className="flex flex-col h-full bg-slate-950">
      {/* Toolbar */}
      <div className="bg-slate-900 border-b border-slate-800 px-6 py-4 flex justify-between items-center sticky top-0 z-40 shadow-lg">
        <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:block">
                <h2 className="text-lg font-bold text-white leading-none">{report.customer?.companyName || 'New Report'}</h2>
                <div className="text-xs text-slate-500 mt-1 font-mono">{report.reportId || 'DRAFT'}</div>
            </div>
        </div>

        <div className="flex items-center space-x-3">
            <span className={`text-xs font-semibold mr-2 ${isError ? 'text-red-500' : 'text-slate-500'}`}>
                {statusMsg}
            </span>
            <button
                onClick={() => saveDraft()}
                disabled={saving}
                className="bg-slate-800 border border-slate-700 text-slate-300 px-4 py-2 rounded-lg shadow-sm hover:bg-slate-700 disabled:opacity-50 text-sm font-medium transition-colors"
            >
                {saving ? '...' : 'Save Draft'}
            </button>
            <button
                ...
                disabled={saving}
                className="bg-red-600 text-white px-5 py-2 rounded-lg shadow hover:bg-red-500 disabled:opacity-50 disabled:cursor-not-allowed flex items-center text-sm font-bold tracking-wide transition-all active:scale-95"
            >
      ...
      <div className="flex-1 overflow-auto p-4 sm:p-8 space-y-6 max-w-6xl mx-auto w-full">
        ...
        <Section title="Technician & Logistics" isOpen={sections.tech} onToggle={() => toggleSection('tech')} icon="fa-user-clock">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Technician</label>
                    <div className="flex items-center bg-slate-800 border border-slate-600 rounded p-2 text-white">
                        <i className="fa-solid fa-user-circle mr-2 text-red-500"></i>
                        {report.technicianName}
                    </div>
                </div>
                <div>
                    <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Arrival Date</label>
                    <input type="date" value={report.arrivalDate} onChange={e => handleChange('arrivalDate', e.target.value)} className="block w-full rounded bg-slate-800 border-slate-600 text-white shadow-sm focus:border-red-500 focus:ring-red-500/20 sm:text-sm" />
                </div>
                <div>
                    <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Departure (Est.)</label>
                    <input type="date" value={report.departureDate} onChange={e => handleChange('departureDate', e.target.value)} className="block w-full rounded bg-slate-800 border-slate-600 text-white shadow-sm focus:border-red-500 focus:ring-red-500/20 sm:text-sm" />
                </div>
            </div>
        </Section>
        ...
        <Section title="Service Classification" isOpen={sections.service} onToggle={() => toggleSection('service')} icon="fa-tags">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {JOB_TYPES.map(type => (
                    <label key={type} className={`flex items-center space-x-2 p-3 border rounded cursor-pointer transition-all ${report.serviceTypes.includes(type) ? 'bg-red-900/30 border-red-600 text-white' : 'hover:bg-slate-800 border-slate-600 text-slate-300'}`}>
                        <input
                            type="checkbox"
                            ...
                            className="text-red-600 focus:ring-red-500 rounded border-slate-500 bg-slate-700"
                        />
        ...
        <Section title="Customer Information" isOpen={sections.customer} onToggle={() => toggleSection('customer')} icon="fa-building">
            <div className="bg-slate-800 p-4 rounded-lg border border-slate-700 mb-6">
                <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Load Existing Customer</label>
                <select onChange={handleCustomerSelect} className="w-full bg-slate-900 border-slate-600 text-white rounded shadow-sm focus:border-red-500 focus:ring-red-500/20 text-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2">
                    <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Site Address</label>
                    <div className="flex gap-2">
                        <textarea rows={2} className="block w-full rounded bg-slate-800 border-slate-600 text-white shadow-sm focus:border-red-500 focus:ring-red-500/20 sm:text-sm" value={report.customer?.address || ''} onChange={e => handleChange('customer.address', e.target.value)} />
                        ...
                        }} className="px-4 bg-slate-700 border border-slate-600 rounded hover:bg-red-600 hover:border-red-600 text-slate-300 hover:text-white transition-colors" title="Verify Address with Gemini Maps"><i className="fa-solid fa-map-location-dot"></i></button>
                    </div>
                </div>
            </div>
        </Section>
        ...
        <Section title="Machine Data" isOpen={sections.machine} onToggle={() => toggleSection('machine')} icon="fa-industry">
            <div className="flex justify-between items-center mb-6 bg-slate-800 p-3 rounded border border-slate-700">
                 <div className="text-xs text-slate-400 font-medium px-2">Ensure Serial # matches physical nameplate</div>
                 <div className="text-right">
                    <button
                        onClick={() => nameplateInputRef.current?.click()}
                        disabled={scanningNameplate}
                        className="flex items-center text-xs font-bold uppercase tracking-wide bg-slate-700 text-red-500 px-3 py-2 rounded border border-slate-600 hover:bg-red-600 hover:text-white hover:border-red-600 disabled:opacity-50 transition-colors shadow-sm"
                    >
            ...
            <div className="mb-4">
                 <div className="flex flex-wrap gap-4">
                    {(report.attachments || []).filter(a => a.fieldRef === 'machineNameplate').map(att => (
                        <div key={att.id} className="relative group w-32 h-24 border border-slate-600 rounded-lg overflow-hidden shadow-lg bg-slate-800">
        ...
        <Section title="Executive Summary / Scope" isOpen={sections.summary} onToggle={() => toggleSection('summary')} icon="fa-file-signature">
            <div className="relative">
                <textarea
                    className="w-full bg-slate-800 border-slate-600 text-white rounded shadow-sm p-4 min-h-[160px] focus:border-red-500 focus:ring-red-500/20 text-sm leading-relaxed placeholder:text-slate-500"
                ...
                <button
                    ...
                    className={`absolute bottom-4 right-4 p-3 rounded-full shadow-lg transition-all ${recording ? 'bg-red-500 animate-pulse text-white' : 'bg-slate-700 text-white hover:bg-red-600'}`}
        ...
        <Section title="Report Issues" isOpen={sections.issues} onToggle={() => toggleSection('issues')} icon="fa-list-ul">
            {report.issues.length === 0 ? (
                <div className="text-center py-10 bg-slate-800 border-2 border-dashed border-slate-600 rounded-lg">
                    <div className="text-slate-600 text-4xl mb-3"><i className="fa-solid fa-clipboard-check"></i></div>
                    <p className="text-slate-400 mb-4 font-medium">No issues documented yet.</p>
                    <button onClick={() => setActiveIssueId('new')} className="text-white bg-red-600 px-4 py-2 rounded shadow hover:bg-red-500 text-sm font-bold">
                ...
                <div className="space-y-4">
                    {report.issues.map((issue, idx) => (
                        <div key={issue.id} onClick={() => setActiveIssueId(issue.id)} className="bg-slate-800 border border-slate-700 rounded-lg p-5 flex justify-between items-center cursor-pointer hover:shadow-lg hover:border-red-600 transition-all group">
                            <div className="flex items-center gap-4">
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${issue.resolved ? 'bg-green-900/50 text-green-400' : 'bg-slate-700 text-slate-300'}`}>
                                    {idx + 1}
                                </div>
                                <div>
                                    <h4 className="font-bold text-white group-hover:text-red-400 transition-colors">{issue.title || 'Untitled Issue'}</h4>
                            ...
                            <div className="flex items-center space-x-3">
                                <span className={`px-2 py-1 rounded text-xs font-bold border ${issue.resolved ? 'bg-green-900/30 text-green-400 border-green-700' : 'bg-slate-700 text-slate-400 border-slate-600'}`}>
                                    {issue.resolved ? 'RESOLVED' : 'OPEN'}
                                </span>
                                <i className="fa-solid fa-chevron-right text-slate-500 group-hover:text-red-500 transition-colors"></i>
                            </div>
                        </div>
                    ))}
                    <button onClick={() => setActiveIssueId('new')} className="w-full py-3 border-2 border-dashed border-slate-600 text-slate-400 rounded-lg hover:border-red-500 hover:text-red-500 font-bold uppercase tracking-wide text-xs transition-colors mt-4">
                        + Add Another Issue
                    </button>
                </div>
            )}
        ...
        <Section title="Follow Up & Suggestions" isOpen={sections.other} onToggle={() => toggleSection('other')} icon="fa-clipboard-list">
            <div className="space-y-8">
                {/* Follow Up */}
                <div className="flex items-center justify-between border border-amber-700 p-5 rounded-lg bg-amber-900/20">
                    <div className="flex items-start gap-3">
                        <div className="text-amber-500 mt-1"><i className="fa-solid fa-bell"></i></div>
                        <div>
                            <span className="font-bold text-white block">Follow Up Required</span>
                            <span className="text-sm text-slate-400">Flag this report for office admin attention</span>
                        </div>
                    </div>
                ...
                {/* Design Suggestion */}
                <div className="border border-slate-700 rounded-lg p-6 bg-slate-800 shadow-lg">
                     <h4 className="font-bold text-white mb-4 border-b border-slate-700 pb-2 flex items-center gap-2">
                        <i className="fa-solid fa-lightbulb text-red-500"></i> Design/Process Suggestion
                     </h4>
...
const Section: React.FC<{title: string, isOpen: boolean, onToggle: () => void, icon?: string, children: React.ReactNode}> = ({title, isOpen, onToggle, icon, children}) => (
    <div className="bg-slate-900 border border-slate-700 rounded-lg shadow-lg overflow-hidden transition-all">
        <div className={`px-6 py-4 flex justify-between items-center cursor-pointer select-none ${isOpen ? 'bg-slate-800 border-b border-slate-700' : 'bg-slate-900 hover:bg-slate-800'}`} onClick={onToggle}>
...
const Input: React.FC<{label: string, value: string, onChange: (v: string) => void}> = ({label, value, onChange}) => (
    <div>
        <label className="block text-xs font-bold text-slate-400 uppercase mb-1 tracking-wide">{label}</label>
        <input type="text" value={value} onChange={e => onChange(e.target.value)} className="block w-full rounded bg-slate-800 border-slate-600 text-white placeholder:text-slate-500 shadow-sm focus:border-red-500 focus:ring-red-500/20 sm:text-sm" />
    </div>
);
...
const TextArea: React.FC<{label: string, value: string, onChange: (v: string) => void, rows?: number}> = ({label, value, onChange, rows=3}) => (
    <div>
        ...
        <textarea
            ...
            className="block w-full rounded bg-slate-800 border-slate-600 text-white placeholder:text-slate-500 shadow-sm focus:border-red-500 focus:ring-red-500/20 sm:text-sm"
        />
    </div>
);
    ...
    return (
        <div className="border border-slate-700 rounded-lg overflow-hidden bg-slate-800 shadow-lg">
            <div className="bg-slate-800 px-4 py-3 border-b border-slate-700 flex justify-between items-center">
                <h4 className="font-bold text-slate-300 uppercase tracking-wide text-xs">Parts Inventory</h4>
                <button onClick={onAdd} className="text-xs bg-slate-700 border border-slate-600 text-slate-300 px-3 py-1 rounded hover:bg-red-600 hover:text-white hover:border-red-600 transition-colors shadow-sm font-medium">
                    <i className="fa-solid fa-plus mr-1"></i> Add Item
                </button>
            </div>

            {parts.length === 0 ? (
                <div className="p-8 text-center text-slate-500 text-sm bg-slate-800">No parts added to this report.</div>
            ) : (
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left">
                        <thead className="bg-slate-700 text-slate-400 font-bold uppercase text-xs">
                            <tr>
                        ...
                        <tbody className="divide-y divide-slate-700 bg-slate-800">
                            {parts.map(part => (
                                <tr key={part.id} className="hover:bg-slate-700">
                                    <td className="p-2"><input className="w-full bg-slate-900 border-slate-600 text-white rounded px-2 py-1 text-sm font-mono placeholder:text-slate-500" value={part.partNumber} onChange={e => onUpdate(part.id, 'partNumber', e.target.value)} placeholder="XXX-XXX" /></td>
                                    <td className="p-2"><input className="w-full bg-slate-900 border-slate-600 text-white rounded px-2 py-1 text-sm placeholder:text-slate-500" value={part.description} onChange={e => onUpdate(part.id, 'description', e.target.value)} placeholder="Part Name" /></td>
                                    <td className="p-2"><input className="w-full bg-slate-900 border-slate-600 text-white rounded px-2 py-1 text-sm text-center" value={part.quantity} onChange={e => onUpdate(part.id, 'quantity', e.target.value)} /></td>
                                    <td className="p-2"><input className="w-full bg-slate-900 border-slate-600 text-white rounded px-2 py-1 text-sm placeholder:text-slate-500" value={part.notes} onChange={e => onUpdate(part.id, 'notes', e.target.value)} placeholder="Optional" /></td>
                                    <td className="p-2 text-center"><button onClick={() => onRemove(part.id)} className="text-slate-500 hover:text-red-500 transition-colors"><i className="fa-solid fa-trash"></i></button></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
...
const StringList: React.FC<{
    ...
    return (
        <div className="border border-slate-700 rounded bg-slate-800 shadow-lg h-full flex flex-col">
            <div className="flex justify-between items-center bg-slate-800 px-3 py-2 border-b border-slate-700 rounded-t">
                <span className="text-xs font-bold text-slate-400 uppercase tracking-wide">{title}</span>
                <button onClick={handleAdd} className="text-red-500 hover:text-red-400 bg-slate-700 border border-slate-600 w-6 h-6 flex items-center justify-center rounded shadow-sm text-xs"><i className="fa-solid fa-plus"></i></button>
            </div>
            <div className="p-3 space-y-2 flex-1 bg-slate-800">
                {items.length === 0 && <p className="text-xs text-slate-500 italic text-center py-2">List is empty</p>}
                {items.map(item => (
                    <div key={item.id} className="flex gap-2">
                        <input
                            className="flex-1 bg-slate-900 border-slate-600 text-white rounded px-2 py-1 text-sm focus:border-red-500 focus:ring-red-500/20 placeholder:text-slate-500"
                            value={item.text}
                            onChange={e => handleUpdate(item.id, e.target.value)}
                            placeholder="Type here..."
                        />
                        <button onClick={() => handleDelete(item.id)} className="text-slate-500 hover:text-red-500 px-1"><i className="fa-solid fa-xmark"></i></button>
                    </div>
                ))}
            </div>
        </div>
